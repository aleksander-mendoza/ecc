...
use num_traits::{AsPrimitive, MulAdd, Num, NumAssign, One, PrimInt, Zero};
use crate::*;
use crate::mat_slice::normalize_mat_columns;
...
#[inline]
pub fn idx_<Idx: Copy+Debug + num_traits::PrimInt+MulAdd<Output=Idx>>(input_pos: &[Idx; 3], kernel_offset: &[Idx; 2], output_idx: Idx, kernel_column: &[Idx; 3], output_volume: Idx) -> Idx {
    let position_within_kernel_column = sub_kernel_offset(input_pos, kernel_offset);
    idx(output_idx, kernel_column.c().idx(position_within_kernel_column), output_volume)
}
...
impl<Idx: Debug + PrimInt + MulAdd<Output=Idx> + NumAssign> ConvShape<Idx> {
    ...
    /**kernel_height * kernel_width*/
    pub fn kernel_column_area(&self) -> Idx {
        self.kernel().c().prod()
    }
    /**kernel_height * kernel_width * in_channels*/
    ...
    pub fn out_volume(&self) -> Idx {
        self.out_shape().c().prod()
    }
    /**in_height * in_width * in_channels*/
    pub fn in_volume(&self) -> Idx {
        self.in_shape().c().prod()
    }
    ...
    pub fn idx(&self, input_pos: &[Idx; 3], output_pos: &[Idx; 3]) -> Idx {
        ...
        debug_assert!(vec_range::contains(&conv::in_range(grid(output_pos), self.stride(), self.kernel()), grid(input_pos)));
        debug_assert!(vec_range::contains(&conv::out_range_clipped(grid(input_pos), self.stride(), self.kernel()), grid(output_pos)));
        idx(self.output_shape().idx(output_pos.c()), self.idx_within_kernel(input_pos, output_pos), self.out_volume())
    }
    ...
    pub fn sparse_dot_slice_<D: AddAssign + Copy>(&self, lhs_tensor: &[Idx], rhs_conv_tensor: &[D], dot_product_output: &mut [D]) where Idx: AsPrimitive<usize> + Step + Hash {
        debug_assert_eq!(rhs_conv_tensor.len(), self.w_shape().prod().as_());
    ...
    pub fn sparse_dot_repeated_slice_<D: AddAssign + Copy>(&self, lhs_tensor: &[Idx], rhs_conv_tensor: &[D], dot_product_output: &mut [D]) where Idx: AsPrimitive<usize> + Step {
        debug_assert_eq!(rhs_conv_tensor.len(), self.minicolumn_w_shape().prod().as_());
        debug_assert_eq!(dot_product_output.len(), self.out_volume().as_());
    ...
    pub fn sparse_unbiased_increment_repeated<D: Copy + Div<Output=D> + AddAssign + FromUsize>(&self, w_slice: &mut [D], epsilon: D, input: &[Idx], output: &[Idx]) where Idx: AsPrimitive<usize> {
        assert_eq!(w_slice.len(), self.minicolumn_w_shape().prod().as_());
    ...
    pub fn sparse_unbiased_increment<D: Copy + Div<Output=D> + AddAssign + FromUsize>(&self, w_slice: &mut [D], epsilon: D, input: &[Idx], output: &[Idx]) where Idx: AsPrimitive<usize> {
        assert_eq!(w_slice.len(), self.w_shape().prod().as_());
        let w_to_increment: Vec<Idx> = Vec::with_capacity(input.len());
    ...
    pub fn repeat_minicolumn<D: Copy>(&self, weights: &[D]) -> Vec<D> where Idx: AsPrimitive<usize> {
        ...
        let mut conv_weights = Vec::with_capacity(i_volume * o_area * channels);
        for i in 0..i_volume {
            for o in 0..o_area {
                for c in 0..channels {
                    debug_assert_eq!(conv_weights.len(),(i * o_area + o) * channels + c);
                    conv_weights.push(weights[i * channels + c])
                }
            }
        }
        conv_weights
    ...
    pub fn add_to_receptive_field_repeated<D:AddAssign+One>(&self, minicolumn_receptive_field:&mut [D], x:&[Idx], y: &[Idx]) where Idx: AsPrimitive<usize> {
        let kc = self.kernel_column_shape().as_::<usize>();
        let c = self.out_channels().as_();
        let kcv:usize = kc.prod();
        assert_eq!(minicolumn_receptive_field.len(),kcv*c);
        let is = self.input_shape().as_::<usize>();
        let os = self.output_shape().as_::<usize>();
        let stride = self.stride().c().as_::<usize>();
        let kernel = self.kernel().c().as_::<usize>();
...
mod tests {
    ...
    fn test1() {
        ...
        let kcv = kc.prod();
        let is = shape.input_shape();
        let os = shape.output_shape();
        for _ in 0..50 {
            // let s:Vec<usize> = rnd_vec( shape.out_volume());
            let kernel_column = rand_set(8, 0..kcv);
            let offset = xy_z3(rnd1::<usize,2>().rem_scalar(S-K+1),0);
            ...
            let rfv = shape.minicolumn_receptive_field_shape().prod();
    ...
    fn test2() {
        ...
        let epsilon = 0.1;
        for _ in 0..50 {
            let s = rnd_vec( shape.out_volume());
            let y = rand_set(8, 0..shape.out_volume());
            let mut u: Vec<f32> = rnd_vec( shape.u_shape().prod());
    ...
    fn test3() {
        ...
        for _ in 0..50 {
            ...
            let y = rand_set(8, 0..shape.out_volume());
            let mut mini_w: Vec<f32> = vec![1.; shape.minicolumn_w_shape().prod()];
            let mut w: Vec<f32> = vec![1.; shape.w_shape().prod()];
    ...
    fn test4() {
        ...
        for _ in 0..50 {
            let x = rand_set(8, 0..shape.in_volume());
            let y = rand_set(8, 0..shape.out_volume());
            let mut mini_w: Vec<f32> = vec![0.; shape.minicolumn_w_shape().prod()];
            let mut w: Vec<f32> = vec![0.; shape.w_shape().prod()];
    ...
    fn test6() {
        let mut rng: StdRng = SeedableRng::seed_from_u64(325);
        let shape = ConvShape::new_in([28, 28, 3], 3, [3, 3], [1, 1]);
        let mini_w: Vec<f32> = rnd_vec(shape.minicolumn_w_shape().prod());
