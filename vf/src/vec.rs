...
pub fn _map_vec<A:SameSize<B>,B:SameSize<A>>(mut v: Vec<A>, f:impl FnMut(A)->B) -> Vec<B> {
    for a in &mut v{
        unsafe{
            let ptr = a as *mut A;
...
pub fn _map_boxed_slice<A:SameSize<B>,B:SameSize<A>>(mut v: Box<[A]>, f:impl FnMut(A)->B) -> Box<[B]> {
    for a in v.as_mut(){
        unsafe{
            let ptr = a as *mut A;
            (ptr as *mut B).write(f(ptr.read()))
...
pub fn _map_boxed_arr<A:SameSize<B>,B:SameSize<A>,const DIM:usize>(mut v: Box<[A;DIM]>, f:impl FnMut(A)->B) -> Box<[B;DIM]> {
    for a in v.as_mut(){
        unsafe{
            let ptr = a as *mut A;
            (ptr as *mut B).write(f(ptr.read()))
