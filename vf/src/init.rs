...
impl <T:Copy,const DIM:usize> InitFoldRev<T> for [T;DIM]{
    fn init_fold_rev(mut end: T, mut f: impl FnMut(T, usize) -> T) -> Self {
        init_fold_rev(end,f)
    }
}
...
pub trait FillFold<T:Copy>{
    fn fill_fold(&mut self, start:T,f: impl FnMut(T, usize) -> T) -> T;
}
impl <T:Copy> FillFold<T> for [T]{
    fn fill_fold(&mut self, mut start: T, mut  f: impl FnMut(T, usize) -> T) -> T{
        for i in 0..self.len(){
            self[i]=start;
            start=f(start,i);
        }
...
pub trait FillFoldRev<T:Copy>{
    fn fill_fold_rev(&mut self, end:T,f: impl FnMut(T, usize) -> T) -> T;
}
impl <T:Copy> FillFoldRev<T> for [T]{
    fn fill_fold_rev(&mut self, mut end: T, mut f: impl FnMut(T, usize) -> T) -> T{
