...
#[pyfunction]
#[text_signature = "(source_image, reference_hist)"]
/// `source_image` is of shape `[height, width, channels]`, `reference_hist` is of shape `[channels, 256]`
pub fn match_precomputed_histogram<'py>(source: &'py PyArray3<u8>, reference_hist: &'py PyArray2<f32>) -> PyResult<&'py PyArray3<u8>>{
    let src_shape = source.shape();
    ...
    let out = vf::histogram::match_precomputed_images(src,&src_shape,rfc);
...
#[pyfunction]
#[text_signature = "(source_image)"]
/// `source_image` shape `[height, width, channels]`
pub fn image_histogram<'py>(source: &'py PyArray3<u8>) -> PyResult<&'py PyArray2<u32>> {
    let src_shape = source.shape();
    assert_eq!(src_shape.len(),3,"Shape should be [height,width,channels]");
    let hist_shape = [src_shape[2],256];
    let src = unsafe{source.as_slice()?};
    let hist = vf::histogram::histograms(src,src_shape[2]);
...
#[pyfunction]
#[text_signature = "(source_image, reference_histograms, blend_policy)"]
/// `source_image` shape `[height, width, channels]`, `reference_histograms` shape `[batch,channels,256]`, return `(matched_image,best_ref_idx,best_square_dist)`.
...
pub fn blend_and_match_best_images<'py>(source: &'py PyArray3<u8>, references: &'py PyArray3<f32>, blend_policy:String) -> PyResult<(&'py PyArray3<u8>,usize,f32,f32)> {
    let src_shape = source.shape();
    let ref_shape = references.shape();
    ...
    let channels = src_shape[2];
    let hist_src = vf::histogram::histograms(src,channels);
    ...
    let out = vf::histogram::match_2precomputed_images(src,&src_shape,hist_src.flatten(),&blended);
    let v = PyArray1::from_vec(source.py(),out.into_vec());
...
pub fn cyclic_group<'py>(py:Python<'py>, n:usize) -> PyResult<&'py PyArray2<usize>> {
    let (m,l) = vf::cyclic_group(n);
    let ll = m.len()/l;
    PyArray1::from_vec(py,m.into_vec()).reshape([ll,l])
}
...
pub fn direct_product<'py>(a: &'py PyArray2<usize>, b: &'py PyArray2<usize>) -> PyResult<&'py PyArray2<usize>> {
    ...
    let (m,g) = vf::direct_product(a_s,a_shape[1],b_s,b_shape[1]);
    PyArray1::from_vec(a.py(),m.into_vec()).reshape([a_shape[0]*b_shape[0],g])
}
...
pub fn learn_uw<'py>(state_space: &'py PyArray2<usize>, w: &'py PyArray2<f32>) -> PyResult<(&'py PyArray3<f32>,&'py PyArray2<f32>)> {
    ...
    vf::learn_u(sp,a_len,w_slice,quotient_monoid_elements,&mut u);
    for a in 0..a_len{
    ...
    vf::learn_w(sp,a_len,w_slice,quotient_monoid_elements,&u,&mut new_w);
